...
class SMSMODEL extends APIRESPONSE
{
    ...
    public function setupsms($data, $loginData)
    {
        ...
        try {
            ...
            // 2. Check if record already exists in cmp_vendor_sms_credentials
            $checkSql = "SELECT id FROM cmp_vendor_sms_credentials WHERE vendor_id = $vendor_id";
            $checkResult = $db->query($checkSql);

            if ($checkResult->num_rows > 0) {
                throw new Exception("SMS credentials already exist for this vendor.");
            }
    ...
    private function getSMSConfigStatus($data, $loginData)
    {
        try {
            ...
            $smsQuery = "SELECT sender_id, entity_id, api_key FROM cmp_vendor_sms_credentials WHERE vendor_id = '$vendor_id'";
